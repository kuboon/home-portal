...
- **画像 post（添付）**: 本文に画像 1 枚を添付（キャプション任意）。画像は
  **storage.kbn.one** に保存（stamp と同経路）。10MB・最大辺 4096px（超過は
  クライアントで縮小、GIF は拒否）。composer の添付ボタン + プレビュー、
  タイムライン描画（タップで原寸）、repost プレビューのサムネイル。画像付き post
  は編集不可（添付が消えるため）。詳細は下記。
- **ナビ / ランディング**: `/`（未ログイン）は一般ユーザー向けの**機能紹介**
  （ヒーロー＋機能カード）で、唯一の導線は「サインイン → ホーム一覧」。
  グローバルナビは「ブランド／ホーム一覧／サインイン／テーマ」のみ。**各種設定は
  すべて各ホーム内**（chat の ⚙ 設定）に集約: `/homes` は一覧＋作成／参加だけの
  軽いハブにし、メンバー／テーマ／招待の管理と、エージェント・通知（Web Push /
  ホーム画面追加）の入口はホーム内設定へ。PWA 追加・通知はホームごと（per-home
  manifest）。
...
チャットの本文に画像 1 枚を添付できる（`kind='stamp'` の単独スタンプとは別で、
`kind='normal'` の通常 post に画像が付く形。キャプション本文は任意）。投稿・
Repost・削除・リアルタイム配信・レート制限・通知は既存メッセージ経路に乗る。

### データモデル — `0013_message_images.sql`

- `messages` に `image_key`（storage.kbn.one のオブジェクトキー）/ `image_type`
  / `image_w` / `image_h` を追加。`image_key IS NULL` = 画像なし。1 メッセージ 1
  枚（MVP）。`Message.image` / repost プレビューの `repost.image` に join なしで
  載る（`m.*` と原本の別名列）。
- 画像付き `kind='normal'` post は**編集不可**（前方 edit は body だけを再 post
  するため添付が失われる。`editMessage` が `image_key IS NULL` を要求）。
  削除（tombstone / hidden）時は body 同様に画像も伏せる。

### 画像の取り扱い — storage.kbn.one

スタンプと同じくブラウザ直アップロード（`client/storage.ts` の
`uploadPostImage`）。**10MB・最大辺 4096px**をクライアントで検証し、超過ラスタは
canvas で縮小（PNG は透過保持、他は JPEG 再エンコード）。**GIF は縮小で
アニメーションが壊れるため、4096px 超なら拒否**（4096px 以内はそのまま = アニメ
保持）。表示は `GET /download?key=…` を DPoP 付き fetch → **blob URL**（key 毎に
...
- 複数枚添付は未対応（1 枚）。ギャラリー表示・原寸ライトボックスも簡易
  （原寸は新規タブ）。
